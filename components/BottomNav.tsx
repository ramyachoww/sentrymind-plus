...
const NavIconWrapper: React.FC<{ children: React.ReactNode; isActive: boolean }> = ({ children, isActive }) => (
  <div className={`w-5 h-5 transition-colors duration-200 ${isActive ? 'text-[#3A5A40] dark:text-green-300' : 'text-gray-400 dark:text-gray-500'}`}>
    {children}
  </div>
);

const NavItem: React.FC<NavItemProps> = ({ label, icon, isActive, onClick }) => (
  <button onClick={onClick} className="flex flex-col items-center justify-center gap-1 flex-1 p-1 focus:outline-none">
    <NavIconWrapper isActive={isActive}>{icon}</NavIconWrapper>
    <span className={`text-[10px] capitalize transition-colors duration-200 ${isActive ? 'text-[#3A5A40] dark:text-green-300 font-semibold' : 'text-gray-500 dark:text-gray-400'}`}>
      {label}
    </span>
  </button>
);
...
const BottomNav: React.FC<BottomNavProps> = ({ currentPage, setPage }) => {
  ...
  return (
    <div className="absolute bottom-0 left-0 right-0 w-full max-w-sm mx-auto bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm border-t border-gray-200 dark:border-gray-700 shadow-t-lg">
